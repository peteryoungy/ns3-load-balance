...
//    double START_TIME = 0.0; simulation
//    double END_TIME = 0.25;
//     long flowCount = 0;
//    long totalFlowSize = 0;
//    uint32_t applicationPauseThresh = 0;
//    uint32_t applicationPauseTime = 1000;
//    double FLOW_LAUNCH_END_TIME = 0.1; "FlowLaunchEndTime", "End time of the flow launch period"
//    double requestRate = load * LEAF_SERVER_CAPACITY *
//    SERVER_COUNT / oversubRatio / (8 * avg_cdf (cdfTable)) / SERVER_COUNT;
...
//1.程序遍历所有的主机，该函数每次都随机选择一个主机作为其数据的发送终点
// 并随机选择端口号rand_range (PORT_START, PORT_END)发送
//2.startTime<FLOW_LAUNCH_END_TIME(0.1)
//3.对于每一次循环 flowcount都加1
//4.对于每一次循环，都随机产生 uint32_t flowSize = gen_random_cdf (cdfTable);即该次发送流量的大小
// 每一次的值可能都不一样
//totalFlowSize += flowSize;保存了遍历完所有主机后，发送流量的总大小
...
//  在哪用的？ 含义？
    ...
    //runmodetr-->runmode
    //为什么不直接使用runmode? --因为有LINK_COUNT限制
    //LINK_COUNT链路数量 其含义？
    //为什么有的协议对LINK_COUNT是否为1有要求？
    RunMode runMode;
...
//  "Whether TCP will pause in TLB & FlowBender" 默认 设置为false
    if (tcpPause)
    {
...
//安装网络协议栈
//因为网络协议栈的安装是基于节点的，所以可以不用必须安排在网络设备安装后面，只要已经创建好节点即可
    NS_LOG_INFO ("Install Internet stacks");

//创建安装网络协议栈的Helper类对象
    InternetStackHelper internet;

//自己定义的类   Ipv4CongaRoutingHelper
...
//猜测1：安装网络协议栈时，使用的是当前已经设置过的（或者默认的）路由协议
//通过internet.SetRoutingHelper（x）不断切换协议
//通过internet.Install（nodes）给该节点安装当前切换到的协议

//猜测2：Ipv4ListRoutingHelper的用处就是 为了结合（同时使用）多种路由协议
    if (runMode == CONGA || runMode == CONGA_FLOW || runMode == CONGA_ECMP
        //serve:static leaf,spine:conga
    {
        ...
        //serve:drb,global leaf,spine:xpath global
    {
        if (runMode == DRB)
        {
        ...
        //serve:tlb leaf,spine:xpath global
    {
        ...
        //sesrve:clove leaf,spine:xpath,global
    {
        ...
        //serve:static leaf,spine:drill
    {
        ...
        //serve:static leaf,spine:letflow
    {
        internet.SetRoutingHelper (staticRoutingHelper);
        internet.Install (servers);
...
//drill conga conga_flow conga_ecmp letflow
//tlb clove
//presto weighted presto  drb
//flowblender
...
//安装信道、分配ip地址
    ...
    // Setting servers
    //一共有两段链路需要配置：serve-leaf,leaf-spine,两段链路参数在理论上可以不同，因此我们分别来配置
    ...
    if (transportProt.compare ("Tcp") == 0)
    {
     	...
     	//DropTailQueue，在溢出时丢弃尾端数据包的FIFO数据包队列。
     	//设置队列类型，属性，属性值
    }
    else
    {
    //思考：为什么要设置？ ECMP以及以前的程序中都从来没有设置过
...
//----------创建点对点链路完成
            ...
            //ipv4地址分配
            //实在是没搞懂为什么地址分配要以netDeviceContainer做参数---现在搞懂了吧
            //就是因为混淆了netDevice 和network device的概念
            //没搞懂ipv4interfave的含义-----
...
//              uint32_t letFlowFlowletTimeout = 500;
                ...
                //思考:为什么conga在此处并没有设置congaFlowletTimeout这个参数？
            }

//tlb和clove的形式一样 但是看不太懂
            if (runMode == TLB)
                //serve:tlb leaf,spine:xpath global
            {
...
//这里设置了conga conga_flow conga_ecmp drill letflow
//          tlb clove
//为什么其他的没有设置？
//--猜测：可能就是与网络协议栈的配置有关
...
//  p2p.SetDeviceAttribute ("DataRate", DataRateValue (DataRate (SPINE_LEAF_CAPACITY)));
    ...
    // set< (A, B) > Leaf A -> Spine B is asymmetric
    std::set<std::pair<uint32_t, uint32_t> > asymLink;
...
//	        在 leaf 端设置enableecmpmode
        ...
        for (int l = 0; l < LINK_COUNT; l++)
        {
            ...
            //链路的非对称性-->spineLeafCapacity的影响
            //isAsymCapacity=flase -->isAsymCapacity=true  两个触发条件含义？
            ...
            //如果因为触发上面两个条件之一 使得 isAsymCapacity=true,则进入下面程序
            if (isAsymCapacity)
            {
...
//              相当于创建了一个容器对象asymLink，里面存放元素的类型是std::pair<uint32_t, uint32_t> ，即pair对
//              使用insert方法 添加元素
...
//-------------- 1.建立leaf-spine点对点链路，使用人为规定的datarate
//               2.为该点对点链路分配ip地址
            ...
            if (runMode == TLB || runMode == DRB || runMode == PRESTO || runMode == WEIGHTED_PRESTO || runMode == Clove)
            {
                ...
                leafToSpinePath[leafToSpine] = netDeviceContainer.Get (0)->GetIfIndex ();//leaf 的端口号

                std::pair<int, int> spineToLeaf = std::make_pair<int, int> (j, i);
                spineToLeafPath[spineToLeaf] = netDeviceContainer.Get (1)->GetIfIndex ();//spine的端口号
//          映射完成
//          思考：这样映射的目的是什么
            }
            ...
            //如果运输层协议为TCP，则清除交换机（leaf switch and spine switch）的队列磁盘
		    if (transportProt.compare ("Tcp") == 0)
            {
                tc.Uninstall (netDeviceContainer);
            }
...
//		             创建从当前leaf 出发的conga route helper对象 路径 addroute to other leaves
//                   源ip地址 目的ip地址 子网掩码 源端口号
                }
...
//              设置了几个参数 settdre setalpha setlinkCapacity 其含义？

                if (runMode == CONGA_ECMP)
		        {
	    		    congaSpine->EnableEcmpMode ();
		        }
//              在 spine 端执行 enableecmpmode() 其含义？
...
//              前面已经设置congaLeaf->SetLinkCapacity(DataRate(SPINE_LEAF_CAPACITY));
//                          congaSpine->SetLinkCapacity(DataRate(SPINE_LEAF_CAPACITY));
//                          bool congaAwareAsym = true;
//              如果 isAsymCapacity=true,将congaLeaf->SetLinkCapacity和congaSpine->SetLinkCapacity
//              设为spineLeafCapacity（ 1/5 SPINE_LEAF_CAPACITY）

//              即：如果非对称（ isAsymCapacity=true）,则将链路降速
//              congaAwareAsym =true含义应该是 conga可以感知到链路的非对称状态
	        }


//          DRILL 配置与上面 conga 一部分代码一样 同样创建了从i-->other i,j-->i的 route 路径
            if (runMode == DRILL)
            {
...
//	        LetFlow与DRILL的唯一区别是 多设置了一个 flowletimeout参数
            if (runMode == LetFlow)
            {
...
//----------配置conga conga_flow conga_ecmp drill letflow 并没有tlb clove
...
//        //serve,leaf,spine:global
    ...
    //猜测：可能是因为只有以上的这些runmode  配置了Ipv4GlobalRouting协议
    //猜测：因为ECMP FlowBender只使用Ipv4GlobalRouting,所以不需要再配置
    //下面的配置是对上面没配置的runmode的补充
    //conga conga_ecmp conga_flow drill clove
    //drb presto weighted_presto clove tlb
    //划掉ecmp flowblender
...
//                  创建一个ipv4drbroutinng 对象的指针 drbRouting 接下来配置路由
...
//                      如果触发条件 则添加weighted路由路径
//                      思考：这个条件的含义？
//set.insert(),将括号里的元素放入容器中（set容器会根据其值进行自动排序）
//set.find(),返回括号元素的迭代器
//set.count（），返回括号中元素的数量，因为set容器中的值不可重复，所以只能是0或者1
...
//                      配置presto路由路径 相比drb增加了一个参数 PRESTO_RATIO  其值为10 其作用？
                    }
                }
            }
        }
...
//    cmd.AddValue ("cdfFileName", "File name for flow distribution", cdfFileName);
    ...
    if (randomSeed == 0)
    {
        srand ((unsigned)time (NULL));//产生随机数 randomSeed==0 时，真正的产生随机数
    }
    else
    {
        srand (randomSeed);//randomSeed！=0时，产生固定值
    }
    //srand(randomseed),根据种子产生随机数序列，种子相同，产生的随机数序列也相同
    //rand()，从产生的随机数序列中按顺序取数，种子相同，取出的序列也是相同的。

    //time函数可以获取当前的系统时间，返回的结果是一个time_t类型，其实就是一个大整数，
    //其值表示从CUT（Coordinated Universal Time）时间1970年1月1日00:00:00（称为UNIX系统的Epoch时间）到当前时刻的秒数。
//  srand((unsigned) time(&t)); 还有一个经常用法，不需要定义time_t型t变量,
...
//安装应用程序
    ...
    for (int fromLeafId = 0; fromLeafId < LEAF_COUNT; fromLeafId ++)
    {
        install_applications(fromLeafId, servers, requestRate, cdfTable, flowCount, totalFlowSize, SERVER_COUNT,
                              LEAF_COUNT, START_TIME, END_TIME, FLOW_LAUNCH_END_TIME, applicationPauseThresh,
